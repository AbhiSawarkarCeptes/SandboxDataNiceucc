trigger leadroundRobin on Lead ( before update,before insert) {
    //
   ...
   /* Map<Integer, Id> queueIds = new Map<Integer, Id>();   //Trigger index --> Queue ID
    ...
    if(Trigger.isUpdate || trigger.isInsert) {        
        ...
        {
            if(string.valueOf(cs.OwnerId).startsWith('00G') && (trigger.oldMap == null || trigger.oldMap.get(cs.Id).ownerID != cs.ownerID))
            {
    ...
    for (Integer i : queueIds.keySet())
    {
        ...
        {   
            ...
            Trigger.new[i].Presales_Agent__c = ag.User__c;

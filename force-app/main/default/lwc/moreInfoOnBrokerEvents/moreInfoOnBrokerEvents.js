...
import getCollataralFiles from '@salesforce/apex/PortalBrokerEventsController.getCollataralFiles';
...
export default class MoreInfoOnBrokerEvents extends LightningElement {
    @api eventId;
    @api eventInfo;
    ...
    collateralFiles(value) {
        {
            ...
            if (data && this.parsedMessage) {
                ...
                this.collateralList = data.map(content => {
                    let modifiedData;
                    if (content.ContentDocument.FileType == "PDF") {
                        modifiedData = { ...content.ContentDocument, isPdf: true };
                    } else if (content.ContentDocument.FileType == "PNG") {
                        modifiedData = { ...content.ContentDocument, isPng: true };
                    } else {
                        modifiedData = { ...content.ContentDocument, isJpg: true };
                    }
                    modifiedData = { ...modifiedData, downloadURL: `${this.parsedMessage.baseUrl}/sfc/servlet.shepherd/document/download/${content.ContentDocumentId}` };
                    return modifiedData;
                });

...
import getobject from '@salesforce/apex/ProformaInvoiceEnqController.objectName';
import callopportunity from '@salesforce/apex/ProformaInvoiceEnqController.getOpportunity';
import callproperty from '@salesforce/apex/ProformaInvoiceEnqController.getProperty';
import callbooking from '@salesforce/apex/ProformaInvoiceEnqController.getBooking';
import calculate from '@salesforce/apex/ProformaInvoiceEnqController.calculate';
import createandsubmit from '@salesforce/apex/ProformaInvoiceEnqController.createAndSubmit';
import updateParent from '@salesforce/apex/ProformaInvoiceEnqController.updateParent';
import generatePDFForBooking from '@salesforce/apex/ProformaInvoiceEnqController.generatePDFForBooking';
import sendEmailAfterApproval from '@salesforce/apex/ProformaInvoiceEnqController.sendEmailAfterApproval';
import checkPDFGeneration from '@salesforce/apex/ProformaInvoiceEnqController.checkPDFGeneration'


export default class ProformaInvoiceEnq extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    //@track isActiveBasicCloseDate = false;
    //contains a boolean value to check if Is Active Basis Closed Date exists on the Opportunity record
    ...
    connectedCallback() {
        if (this.recordId) {
            ...
            getobject({
                ...
                if (result == 'PropStrength__Request__c') {
                    ...
                    callopportunity({
                        ...
                        .then(result => {
                            ...
                            console.log('result1___');
                            console.log(result);
                            
                            callproperty({
                                recId: result.PropStrength__Selected_Property_Ids__c
                            }).then(result2 => {
                                console.log('result2___');
                                console.log(result2);
                                console.log(result2.hasOwnProperty('PropStrength__Property_Charges__r'));
                                if (result2.hasOwnProperty('PropStrength__Application_Booking__r')) {
                                    this.bookingLookup = result2.PropStrength__Application_Booking__r[0].Id;
                                }
                                if(result2.hasOwnProperty('PropStrength__Property_Charges__r')) {
                                    var total = 0;
                                    for (var i = 0; i < result2.PropStrength__Property_Charges__r.length; i++) {
                                        var prOth = result2.PropStrength__Property_Charges__r[i];
                                        console.log(prOth);
                                        if(result2.hasOwnProperty('PropStrength__Super_Area__c') && prOth.hasOwnProperty('PropStrength__Rate_per_unit_Area__c') && prOth.PropStrength__Rate_per_unit_Area__c != null) {
                                            total += (prOth.PropStrength__Rate_per_unit_Area__c * result2.PropStrength__Super_Area__c);
                                        } else if(prOth.hasOwnProperty('PropStrength__Fixed_Charge__c') && prOth.PropStrength__Fixed_Charge__c != null) {
                                            total += prOth.PropStrength__Fixed_Charge__c;
                                        }
                                    }
                                    console.log('total*****');
                                    console.log(total);
                                    this.purchaseprice = total + result2.PropStrength__Total_Sales_Consideration__c;
                                }
                            })
                            .catch(error => {
                                console.log('-----error-----' + JSON.stringify(error));
                            ...
                            if (result.PropStrength__Selected_Property_Ids__c == null) {
                    ...
                    callbooking({
                        ...
                        .then(result => {
                            this.parentdata = result;
                            if (result.PropStrength__Property__c == null) {
                                this.unitexist = false;
                            } else {
                                // this.purchaseprice = result.Agreement_Value__c;

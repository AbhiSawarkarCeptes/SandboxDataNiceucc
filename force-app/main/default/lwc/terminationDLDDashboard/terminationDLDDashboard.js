...
import getProjects from '@salesforce/apex/TerminationDLDLWCController.getProjects'
import getBookings from '@salesforce/apex/TerminationDLDLWCController.getBookings';
import updateAllocation from '@salesforce/apex/TerminationDLDLWCController.updateAllocation';
import updateBookingRow from '@salesforce/apex/TerminationDLDLWCController.updateBookingRow';
import returnToCollections from '@salesforce/apex/TerminationDLDLWCController.returnToCollections';
import TIME_ZONE from '@salesforce/i18n/timeZone';
import approveFinalNotice from '@salesforce/apex/TerminationDLDLWCController.approveEslamFinalNotice';
import validateEslamFinalNotice from '@salesforce/apex/TerminationDLDLWCController.validateEslamFinalNotice';
import sendEslamFinalNotice from '@salesforce/apex/TerminationDLDLWCController.sendEslamFinalNotice';
import sendEslamDLDNotice from '@salesforce/apex/TerminationDLDLWCController.sendEslamDLDNotice';
import validateEslamDLDNotice from '@salesforce/apex/TerminationDLDLWCController.validateEslamDLDNotice';
import isDLDProfile from '@salesforce/apex/TerminationDLDLWCController.isDLDProfile';
...
const columns1 = [
    ...
    { label: 'SPA Status', initialWidth: 150, fieldName: 'spaexecuted', type: 'text', editable: false },
    { label: 'Pre-Registration Status', initialWidth: 150, fieldName: 'preRegstatus', type: 'text', editable: false },
    { label: 'Title Deed Status', initialWidth: 150, fieldName: 'titleDeedStatus', type: 'text' ,editable: false},
    { label: 'Sale Value', initialWidth: 110, fieldName: 'saleValue', type: 'text',editable: false },
    { label: 'Collected Amount', initialWidth: 110, fieldName: 'collected', type: 'text' ,editable: false},
    { label: 'Collected %', initialWidth: 100, fieldName: 'collectedpercentage', type: 'percentage' ,editable: false},
    { label: 'Due Amount', initialWidth: 110, fieldName: 'fullDue', type: 'text',editable: false },
    { label: 'Due %', initialWidth: 100, fieldName: 'duepercentage', type: 'percentage',editable: false },
    { label: 'Audit Approved Date', initialWidth: 180, fieldName: 'auditapproveddate', type: 'date',editable: false },
    { label: 'Audit Approved Date1', initialWidth: 180, fieldName: 'auditapproveddate', type: 'date',editable: true },
    ...
    {
        label: 'DLD Notice Status Category', initialWidth: 180, fieldName: 'dldNoticeStatusCategory', type: 'picklist', typeAttributes: {
    ...
    { label: 'Comments/Remarks', initialWidth: 180, type: 'textfld',fieldName: "commentsremarks", wrapText: true, editable: true, typeAttributes: {                
             label: 'CommentsRemarkslabel',
             remark: { fieldName: 'commentsremarks' } // default value for picklist  
             , context: { fieldName: 'bookingId' }          
        } },

    { label: 'Allow PR', initialWidth: 180, type: 'booleanfld',fieldName: "allowPR", wrapText: true, editable: true, typeAttributes: {                
             label: 'flags',
             flag: { fieldName: 'allowPR' } // default value for picklist  
             , context: { fieldName: 'bookingId' }          
        } },
    { label: 'Termination Remarks', initialWidth: 180, type: 'textfld',fieldName: "terminationremarks", wrapText: true, editable: true, typeAttributes: {                
             label: 'terminationremarkslabel',
             remark: { fieldName: 'terminationremarks' } // default value for picklist  
             , context: { fieldName: 'bookingId' }          
        } },
...
const columns3 = [
    ...
    { label: 'Audit Approved Date', initialidth: 180, fieldName: 'auditapproveddate', type: 'date' },
    { label: 'Allow PR', initialWidth: 180, type: 'booleanfld',fieldName: "allowPR", wrapText: true, editable: true, typeAttributes: {                 
             label: 'flags',
             flag: { fieldName: 'allowPR' } // default value for picklist  
             , context: { fieldName: 'bookingId' }          
    ...
    { label: 'Developer Final Notice Sent Date (Courier)', initialWidth: 180, fieldName: 'developerFinalNoticeSentDateCourier', type: 'dateFld', editable: true, typeAttributes: {                 
             label: 'developerFinalNoticeSentDateCourierlabel',
             flag: { fieldName: 'developerFinalNoticeSentDateCourier' }, // default value for picklist  
             context: { fieldName: 'bookingId' }          
        } },    
   ...
   { label: 'Developer Final Notice Receive Date (Courier)', initialWidth: 180, fieldName: 'finalNoticeReceivedDate', type: 'dateFld', editable: true, typeAttributes: {                 
             label: 'finalNoticeReceivedDatelabel',
             flag: { fieldName: 'finalNoticeReceivedDate' }, // default value for picklist  
             context: { fieldName: 'bookingId' }          
        } }, 
    ...
    { label: 'Forfeited Amount (if unit terminated)', initialWidth: 180, type: 'textfld',fieldName: "forfeitedAmount", wrapText: true, editable: true,
             typeAttributes: {                
             label: 'forfeitedAmountlabel',
             remark: { fieldName: 'forfeitedAmount' } // default value for picklist  
             , context: { fieldName: 'bookingId' }          
        } 
    },
    
    {
        label: 'Amount paid by Customer', initialWidth: 200, editable: true, fieldName: 'amountPaidByCustomer', type: 'picklist',
        typeAttributes: {
            label: 'amountPaidByCustomerlabel',
            placeholder: 'Amount paid by Customer', options: [
    ...
    {
        label: 'Submit', initialWidth: 180, type: 'button', typeAttributes: {
            label: 'Submit',
            name: 'Submit',
            title: 'Submit',
            value: '',
...
export default class TerminationDLDDashboard extends NavigationMixin(LightningElement) {
    ...
    handleRowAction(event) {
        ...
        switch (action.name) {
            ...
            case 'Send Final Notice':
                ...
                break;
            case 'Send DLD Notice':
                console.log('Send DLD Notice');
                console.log('rowDate');
                console.log(rowData);
                this.tempRow = rowData;
                validateEslamDLDNotice({ row: rowData })
                    .then(result => {
                        console.log(result);
                        if (result == 'Unauthorized') {
                            const evt = new ShowToastEvent({
                        ...
                        if (result == 'Success') {
                            this.isEmailModalOpenforDLD = true;
                        }
                        this.draftValues = [];
                    })
                    .catch(error => {
            ...
            rowsStr.push(row);
            console.log(rowsStr);
            updateBookingRow({ row: JSON.stringify(rowsStr), isSubmitprocess: true })
                .then(result => {
                    console.log();
                    if (result) {
                        const evt = new ShowToastEvent({
    ...
    sendDLDNotice() {
        ...
        console.log('Send sendFinalNotice : ' + this.emailValues);
        this.isEmailModalOpenforDLD = false;

        sendEslamDLDNotice({ row: this.tempRow, emails: this.emailValues })
            .then(result => {
                console.log('Sending DLD notice....');
                console.log(result);
                if (result == 'Success') {
                    const evt = new ShowToastEvent({
                        variant: 'success',
                        message: 'DLD Notice Sent successfully!',
                    });
                    this.dispatchEvent(evt);
                } else {
                    console.log('Sending DLD notice....');
                    ...
                    const evt = new ShowToastEvent({
                        variant: 'error',
                        message: 'Error sending the DLD Notice!',
                    });
                    this.dispatchEvent(evt);
                }
    ...
    handleProjectChange(event) {
        ...
        if(event.detail.value == 'All'){
            this.loadBookings(event.detail.value, 'All', this.selectedType);
        }else{
    ...
    datechanged(event){
        let dataRecieved = event.detail.data;        
        console.log('*---***---');
        console.log(dataRecieved.flag);
        console.log(dataRecieved.label); 
        let updatedItem = { bookingId: dataRecieved.context, value: dataRecieved.flag, label: dataRecieved.label };
        let rows = [];
        rows.push(updatedItem);
        updateBookingRow({ row: JSON.stringify(rows), canceltheprocess: false, isdate: true })
                .then(result => {
                    console.log(result);
                    if (result) {
                        const evt = new ShowToastEvent({
    ...
    flagchanged(event){
        ...
        let rows = [];
        rows.push(updatedItem);
        updateBookingRow({ row: JSON.stringify(rows), canceltheprocess: false, isboolean: true })
                .then(result => {
                    console.log(result);
                    if (result) {
                        const evt = new ShowToastEvent({
    ...
    textChanged(event){
        let dataRecieved = event.detail.data;        
        console.log('*---***---');
        console.log(dataRecieved.remark);
        console.log(dataRecieved.label); 
        let isCommentChange = false;
        let updatedItem = { bookingId: dataRecieved.context, value: dataRecieved.remark, label: dataRecieved.label };
        if(dataRecieved.label == 'terminationremarkslabel'){  
          this.updatedRemarkItem.forEach(each => {
                console.log(each.bookingId,'**----**',updatedItem.bookingId);
          ...
          this.updatedRemarkItem.push({ bookingId: dataRecieved.context, value: dataRecieved.remark, label: dataRecieved.label });
          console.log(this.updatedRemarkItem);  
        }
        
        let rows = [];
        rows.push(updatedItem);
        updateBookingRow({ row: JSON.stringify(rows), canceltheprocess: false, istext: true })
                .then(result => {
                    console.log(result);
                    if (result) {
                        const evt = new ShowToastEvent({
    ...
    picklistChanged(event) {
        ...
        if (dataRecieved.value == 'Allocate to Collection Team') {
            ...
            let isCommentChange = false;
            console.log(this.validationForRemark);
            this.updatedRemarkItem.forEach(each => {
                console.log(each.bookingId,'**----**',updatedItem.bookingId);
                if (each.bookingId == updatedItem.bookingId) {
                    console.log('Matched done');

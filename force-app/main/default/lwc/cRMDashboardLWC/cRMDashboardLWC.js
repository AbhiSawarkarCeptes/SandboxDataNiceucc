...
import getProjects from '@salesforce/apex/CRMDashboardControllerClone.getProjects';
import getBookings from '@salesforce/apex/CRMDashboardControllerClone.getBookings';
import updateAllocation from '@salesforce/apex/CRMDashboardControllerClone.updateAllocation';
import updateBookingRow from '@salesforce/apex/CRMDashboardControllerClone.updateBookingRow';
import sendFinalNotice from '@salesforce/apex/FinalNoticeBookingController.sendFinalNotice';
import sendForfeitureNotice from '@salesforce/apex/FinalNoticeBookingController.sendForfeitureNotice';
...
const columns1 = [
    ...
    { label: 'Due Amount', fieldName: 'due', type: 'text' },
    ...
    { label: 'Comments/Remarks', initialWidth: 180, fieldName: 'commentsremarks', type: 'text', editable: true },
    {
        label: 'Allocation', initialWidth: 180, editable: true, fieldName: 'cancellationAllocation', type: 'picklist', typeAttributes: {
            ...
            placeholder: 'Allocation', options: [
                ...
                { label: 'Allocate to Account Team', value: 'Allocate to Account Team' },
                { label: 'Allocated to MD office', value: 'Allocated to MD office' },
...
const columns2 = [
    ...
    { label: 'Sale Value', fieldName: 'saleValue', type: 'text' },
    { label: 'Collected Amount', fieldName: 'collected', type: 'text' },
    { label: 'Collected %', fieldName: 'collectedpercentage', type: 'text' },
    { label: 'Due Amount', fieldName: 'due', type: 'text' },
    { label: 'Due %', fieldName: 'duepercentage', type: 'text' },
   ...
   /* {
        label: '', fieldName: 'finalNoticeReviewLink', initialWidth: 180, type: 'url', target: '_blank', typeAttributes: {
            label: 'Review Final Notice',
        },
        hideDefaultActions: true
    },*/
    {
        label: 'Review Final Notice', initialWidth: 180, type: 'button', typeAttributes: {
            label: 'Review Final Notice',
            name: 'Review Final Notice',
            title: 'Review Final Notice',
            value: '',
            iconPosition: 'left',
            hideDefaultActions: true
        }
   ...
   /* {
    ...
    {
        label: 'Review Forfeiture Notice', initialWidth: 200, type: 'button', typeAttributes: {
            label: 'Review Forfeiture Notice',
            name: 'Review Forfeiture Notice',
            title: 'Review Forfeiture Notice',
            value: '',
            iconPosition: 'left',
...
const columns3 = [
    ...
    {
        label: 'Final Notice Status', initialWidth: 180, fieldName: 'finalNoticeStatus', type: 'picklist', 
        typeAttributes: {
...
export default class cRMDashboard extends NavigationMixin(LightningElement) {
    ...
    picklistChanged(event) {
        event.stopPropagation();
        let dataRecieved = event.detail.data;
        this.allocatedToMdCase = dataRecieved;
        console.log(dataRecieved.label);
        if (dataRecieved.value != 'Allocated to MD office') {
        ...
        } else if (dataRecieved.value == 'Allocated to MD office') {
            this.openModal = true;
        }
    }
    ...
    handleSubmitModal() {
        ...
        let updatedItem = { bookingId: this.allocatedToMdCase.context, value: this.allocatedToMdCase.value, label: this.allocatedToMdCase.label+'##'+this.reasonValue };
        console.log(updatedItem);
    ...
    handleRowAction(event) {
        ...
        switch (action.name) {
            ...
            case 'Send Forfeiture Notice':
                ...
                break;
            case 'Review Final Notice' :
                var rowDataObj = JSON.parse(rowData);
                var bkId = rowDataObj.bookingId;
                bkId = bkId.split('/')[1];
                let url = '/apex/FinalNoticeBooking?id='+bkId;
                window.open(url, "_blank");
                break;
            case 'Review Forfeiture Notice' :
                var rowDataObj = JSON.parse(rowData);
                var bkId = rowDataObj.bookingId;
                bkId = bkId.split('/')[1];
                let url1 = '/apex/ForfeitureNoticeBooking?id='+bkId;
    ...
    canceltheprocess() {
        updateBookingRow({ row: this.tempRow, canceltheprocess: true, ispicklist: false, cancelComments: this.cancelComments })
            .then(result => {
                console.log(result);
                if (result) {
                    const evt = new ShowToastEvent({

...
export default class MultiPickList extends LightningElement {
    ...
    handleDropdown(event) {
        const getElement = event.target.closest(".ms-picklist-dropdown");
        if (getElement.classList.contains("slds-is-open")) {
            getElement.classList.remove("slds-is-open")
        } else {
            getElement.classList.add("slds-is-open");
        }
        console.log('OUTPUT : handleDropdown', getElement.classList, getElement.classList.contains("slds-is-open"));
    }

    handleCLoseDropdown() {
        setTimeout(() => {
            const getElements = this.template.querySelectorAll(".ms-picklist-dropdown");
            getElements.forEach(getElement => {
                getElement.classList.remove('slds-is-open');
            });
            console.log('OUTPUT : handleCLoseDropdown');
        }, 300)
    }
    onDropDownClick() {
